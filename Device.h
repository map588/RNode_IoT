...
const uint8_t dev_keys[] PROGMEM = {
    0x0f, 0x15, 0x86, 0x74, 0xa0, 0x7d, 0xf2, 0xde, 0x32, 0x11, 0x29,
    0xc1, 0x0d, 0xda, 0xcc, 0xc3, 0xe1, 0x9b, 0xac, 0xf2, 0x27, 0x06,
    0xee, 0x89, 0x1f, 0x7a, 0xfc, 0xc3, 0x6a, 0xf5, 0x38, 0x08};
...
#define DEV_FWHASH_OFFSET                                                      \
  EEPROM_SIZE - EEPROM_RESERVED - DEV_SIG_LEN - DEV_HASH_LEN
...
void calculate_region_hash(unsigned long long start, unsigned long long end,
                           uint8_t *return_hash) {
...
void device_validate_partitions() {
  ...
  calculate_region_hash(APPLICATION_START,
                        APPLICATION_START + retrieve_application_size(),
                        dev_firmware_hash);
#endif
...
bool device_init() {
  if (bt_ready) {
#if MCU_VARIANT == MCU_ESP32
    for (uint8_t i = 0; i < EEPROM_SIG_LEN; i++) {
      dev_eeprom_signature[i] = eeprom_read(eeprom_addr(ADDR_SIGNATURE + i));
    }
